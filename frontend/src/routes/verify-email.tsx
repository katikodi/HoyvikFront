            ...
            <div className="w-full max-w-md">
                <div className="rounded-2xl bg-white p-8 text-center shadow-sm">
                    {(!search.userId || !search.token) && (
                        <ErrorState message={"This verification link is invalid or incomplete."} />
                    )}
                    {isError && (
                        <ErrorState
                            message={`Something went wrong while verifying your email. Please try again later. Error: ${error?.cause}`}
                        />
                    )}
                    {isFetching && <LoadingState />}
                    {!data && !isFetching && (
                        <ErrorState message="We could not verify your email. The link may have expired or already been used." />
                    )}
